...
int8_t adcConfig(uint8_t nSamples, uint8_t AnalogPin){
    ...
    AD1CHSbits.CH0SA = AnalogPin; /* Select AnalogPin as input for A/D converter*/
    AD1PCFG = 0x01 << AnalogPin; /* Set AnalogPin to analog mode*/
...
int8_t adcControl(uint8_t adcrun){
    if (adcrun == 1) {
        ...
        AD1CON1bits.ON = 0; /* Disable A/D module*/
    }
}
